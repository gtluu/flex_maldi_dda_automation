...
from timsconvert.classes import TimsconvertTsfData, TimsconvertTdfData
from timsconvert.data_input import schema_detection
from timsconvert.parse import parse_maldi_tsf, parse_maldi_tdf
...
def preprocess(maldi_spectra, args):
...
def main():
        ...
        for spot_group in autox:
            if spot_group.attrib['sampleName'] == args['blank']:
                ...
                if schema_detection(dot_d_path) == 'TSF':
                    blank = TimsconvertTsfData(dot_d_path, sdk)
                    num_frames = blank.analysis['Frames'].shape[0] + 1
                    scan_dicts = parse_maldi_tsf(blank, 1, num_frames, 'centroid', False, 0, 64)
                elif schema_detection(dot_d_path) == 'TDF':
                    blank = TimsconvertTdfData(dot_d_path, sdk)
                    num_frames = blank.analysis['Frames'].shape[0] + 1
                ...
                maldi_spectra = preprocess(maldi_spectra, args)
    ...
    for spot_group in autox:
        if spot_group.attrib['sampleName'] != args['blank']:
            dot_d_path = os.path.join(autox.attrib['directory'], spot_group.attrib['sampleName'] + '.d')
            if schema_detection(dot_d_path) == 'TSF':
                data = TimsconvertTsfData(dot_d_path, sdk)
                num_frames = data.analysis['Frames'].shape[0] + 1
                scan_dicts = parse_maldi_tsf(data, 1, num_frames, 'centroid', False, 0, 64)
            elif schema_detection(dot_d_path) == 'TDF':
                data = TimsconvertTdfData(dot_d_path, sdk)
                num_frames = data.analysis['Frames'].shape[0] + 1
